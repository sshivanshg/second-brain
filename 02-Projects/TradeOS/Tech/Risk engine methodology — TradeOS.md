...
## The chain
1. **Prices → returns.** Two series, used correctly: **split-adjusted price** (`close`) for levels (SMA / 52-wk) + market value + liquidity, and **total-return** (`adj_close`, splits + dividends) for all returns. **Log returns** for vol/covariance (additive); **simple returns** for VaR/stress/beta (real P&L).
...
7. **Beta:** full-sample OLS βᵢ = cov(i, NIFTY)/var(NIFTY) with **Bloomberg shrinkage** (β_adj = ⅔·raw + ⅓·1). Beta is a *structural* exposure → long window, NOT the short EWMA window used for vol (that made it noise). Portfolio β ≈ 0.89.
